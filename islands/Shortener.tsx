            ...
            {(props?.display) ? (
                <div class="flex items-center justify-center bg-gray-100 p-4" >
                    <div class="w-full max-w-md rounded-md border bg-gray-100 border-gray-400 px-4 py-2 mr-2">
                        {((SITE_URL) ? `https://${SITE_URL}` : "http://localhost:8000") + `/${props?.display}`}
                    </div>
                    <Button
                        onClick={() => navigator.clipboard.writeText(((SITE_URL) ? `https://${SITE_URL}` : "http://localhost:8000") + `/${props?.display}`)}
                    >
